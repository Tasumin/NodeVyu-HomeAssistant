...
class NodeVyuApi:
    ...
    async def async_status(self) -> dict[str, Any]:
        try:
            async with self._session.get(
                f"{self.base_url}/api/integrations/home-assistant/status",
                headers=self._headers,
                timeout=ClientTimeout(total=15),
            ) as response:
                if response.status in (401, 403):
                    raise NodeVyuAuthError("Invalid or unauthorized NodeVyu token")
                if response.status >= 400:
                    raise NodeVyuApiError(f"NodeVyu returned HTTP {response.status}")
    ...
    async def async_events(self, after: int = 0) -> AsyncIterator[dict[str, Any]]:
        """Yield events from NodeVyu's SSE stream until the server rotates it."""
        headers = {**self._headers, "Accept": "text/event-stream"}
        if after:
            headers["Last-Event-ID"] = str(after)
        try:
            async with self._session.get(
                f"{self.base_url}/api/integrations/home-assistant/stream",
                headers=headers,
                ...
                if response.status in (401, 403):
                    raise NodeVyuAuthError("Invalid or unauthorized NodeVyu token")
                if response.status >= 400:
                    raise NodeVyuApiError(f"NodeVyu stream returned HTTP {response.status}")
                event_name = "message"
